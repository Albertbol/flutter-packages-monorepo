...
class ScreenTextExtractorPlugin : public flutter::Plugin
{
  ...
  private:
    ...
    void ScreenTextExtractorPlugin::SimulateCtrlKeyPress(
    ...
    const std::string &key)
{
    ...
    // Set the press of the arg key
    copyText[1].ki.wVk = key;
    copyText[1].ki.dwFlags = 0;
    copyText[1].type = INPUT_KEYBOARD;

    // Set the release of the arg key
    copyText[2].ki.wVk = key;
...
void ScreenTextExtractorPlugin::HandleMethodCall(const flutter::MethodCall<flutter::EncodableValue> &method_call,
                                                 std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result)
{
    if (method_call.method_name().compare("simulateCopyKeyPress") == 0)
    {
        SimulateCtrlKeyPress(method_call, std::move(result), 'C');
    }
    else if (method_call.method_name().compare("simulatePasteKeyPress") == 0)
    {
        SimulateCtrlKeyPress(method_call, std::move(result), 'V');
